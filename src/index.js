 ...
 *      { 
 *          BUILD_ENV, LIBRARY_NAME, R4X_HOME, R4X_TARGETSUBDIR, R4X_ENTRY_SUBFOLDER, SRC_MAIN, SITE_SUBFOLDER, SUBFOLDER_BUILD_MAIN, 
...
const buildConstants = (rootDir, outputFile, overrides) => {
    if (!fs.existsSync(rootDir)) {
        throw Error("ERROR: rootDir (root directory) doesn't exist: " + JSON.stringify(rootDir));
    }
    
    if (!fs.lstatSync(rootDir).isDirectory()) {
        throw Error("ERROR: rootDir (root directory) must be a directory. It doesn't seem to be: " + JSON.stringify(rootDir));
    }
    outputFile = ((outputFile || "") + "").trim(); 
    if (outputFile === '') {
        throw Error("ERROR: outputFile is empty/missing: " + JSON.stringify(outputFile));
    }
    ...
    if (fs.existsSync(outputFile)) {
        if (!(overrides.overwriteConstantsFile || overrides.OVERWRITE_CONSTANTS_FILE)) {
            ...
            verboseLog("\t\tFile exists and overwriteConstantsFile is enabled!");
        }
    } 
